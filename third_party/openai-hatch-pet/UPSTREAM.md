...
The project owns its editable-project model, orchestration, CLI, build lineage,
review gates, and installation safety. Only helpers used by the current runtime
are retained under `petkit/v2scripts/`, so production builds do not depend on a
changing global skill:
...
The upstream cardinal helpers and animation-row reference are not retained:
the project CLI and `petkit/contracts/v2.json` supersede them. `SHA256SUMS`
identifies the exact retained files. The snapshot is provenance only and there
is no runtime synchronization with an installed skill.
